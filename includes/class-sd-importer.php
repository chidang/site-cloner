...
class Importer {
	...
	private function import_sql( $mysqli, $file ) {
		...
		mysqli_query( $mysqli, "SET SESSION sql_mode = '" . mysqli_real_escape_string( $mysqli, $prev_mode ) . "'" ); // phpcs:ignore WordPress.DB.RestrictedFunctions.mysql_mysqli_query -- Restore WP's original sql_mode on its shared connection; see note above.
		return $count;
	}
