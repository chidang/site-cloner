...
class Health {
	...
	public static function report() {
		...
		$rows[] = array(
			...
			$php,
			version_compare( $php, '7.4', '>=' ) ? '' : __( 'PHP ≥ 7.4 is recommended', 'site-cloner' ),
		);

		$mysqli = extension_loaded( 'mysqli' );
		$rows[] = array( 'mysqli', $mysqli ? 'ok' : 'fail', $mysqli ? __( 'present', 'site-cloner' ) : __( 'missing', 'site-cloner' ), __( 'Required to import the database', 'site-cloner' ) );

		$zip = class_exists( 'ZipArchive' );
		$rows[] = array( 'ZipArchive (ext-zip)', $zip ? 'ok' : 'fail', $zip ? __( 'present', 'site-cloner' ) : __( 'missing', 'site-cloner' ), __( 'Required to compress/extract', 'site-cloner' ) );

		$curl = extension_loaded( 'curl' );
		$rows[] = array( 'cURL', $curl ? 'ok' : 'warn', $curl ? __( 'present', 'site-cloner' ) : __( 'missing', 'site-cloner' ), __( 'Needed for "Pull from production"', 'site-cloner' ) );

		$ssl = extension_loaded( 'openssl' );
		$rows[] = array( 'OpenSSL', $ssl ? 'ok' : 'warn', $ssl ? __( 'present', 'site-cloner' ) : __( 'missing', 'site-cloner' ), __( 'Needed for HTTPS', 'site-cloner' ) );

		$rb = function_exists( 'random_bytes' );
		$rows[] = array( 'random_bytes', $rb ? 'ok' : 'fail', $rb ? __( 'present', 'site-cloner' ) : __( 'missing', 'site-cloner' ), __( 'Generates security tokens', 'site-cloner' ) );

		if ( ! is_dir( FLEXA_PACKAGE_DIR ) ) {
			wp_mkdir_p( FLEXA_PACKAGE_DIR );
		}
		$writable = wp_is_writable( FLEXA_PACKAGE_DIR );
		$rows[] = array( __( 'sd-packages directory is writable', 'site-cloner' ), $writable ? 'ok' : 'fail', FLEXA_PACKAGE_DIR, __( 'Where packages are stored/downloaded', 'site-cloner' ) );
		...
		$rows[] = array( __( 'max upload size', 'site-cloner' ), 'info', size_format( $upload ), __( 'Does not affect the "pull by link" flow', 'site-cloner' ) );

		$cfg = ABSPATH . 'wp-config.php';
		$cw  = is_file( $cfg ) && wp_is_writable( $cfg );
		$rows[] = array( __( 'wp-config.php is writable', 'site-cloner' ), $cw ? 'ok' : 'warn', $cw ? __( 'yes', 'site-cloner' ) : __( 'no', 'site-cloner' ), __( 'Needed when changing the table prefix', 'site-cloner' ) );

		$disabled = array_map( 'trim', explode( ',', (string) ini_get( 'disable_functions' ) ) );
		$shell    = function_exists( 'shell_exec' ) && ! in_array( 'shell_exec', $disabled, true );
		$rows[] = array( 'shell_exec (faster mysqldump)', $shell ? 'ok' : 'info', $shell ? __( 'yes', 'site-cloner' ) : __( 'no', 'site-cloner' ), __( 'Optional — without it, export still runs via PHP', 'site-cloner' ) );

		$uploads = wp_upload_dir();
		$free    = @disk_free_space( $uploads['basedir'] );
		if ( false !== $free ) {
			$rows[] = array( __( 'Free disk space', 'site-cloner' ), 'info', size_format( $free ), '' );
		}
	...
	public static function render() {
		...
		?>
		<details class="sd-card sd-health" <?php echo $fails ? 'open' : ''; ?>>
			<summary style="cursor:pointer;font-weight:600;">
				<?php esc_html_e( 'System check', 'site-cloner' ); ?>
				<?php if ( $fails ) : ?>
					<?php /* translators: %d: number of items that need attention */ ?>
					<span style="color:#dc3232;">— <?php echo esc_html( sprintf( _n( '%d item needs attention', '%d items need attention', $fails, 'site-cloner' ), $fails ) ); ?></span>
				<?php else : ?>
					<span style="color:#46b450;">— <?php esc_html_e( 'OK', 'site-cloner' ); ?></span>
				<?php endif; ?>
			</summary>
